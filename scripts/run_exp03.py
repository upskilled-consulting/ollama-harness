"""
run_exp03.py — execute the 9-run CRD for experiment-03.

Runs agent.py for each task in the randomized order defined in experiment-03.md.
Each run appends one record to runs.jsonl. Run analyze_exp03.py after completion.

Usage:
    conda activate ollama-pi
    python run_exp03.py
    python run_exp03.py --resume   # skip tasks whose output file already exists
"""
...
# Randomized run order from experiment-03.md
RUN_ORDER = ["T_C", "T_A", "T_C", "T_B", "T_A", "T_B", "T_C", "T_B", "T_A"]
...
def main():
    ...
    print("\n" + "="*60)
    print(" Experiment-03: Evaluator Upgrade Impact")
    print(f" {len(RUN_ORDER)} runs  |  evaluator: Qwen3-Coder:30b  |  threshold: 8.0")
    if resume:
